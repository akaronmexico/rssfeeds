...
const getDetailedFieldedHistogram = async (fieldName, fieldValue, dateRangeStr, interval = '1h') => {
  const searchBody = {
    index: "partnerdata",
    type: "partnerarticle",
    body: {
  ...
  for(let i = 0; i < buckets.length; i++) {
    ...
    for(let j = 0; j < entryBuckets.length; j++) {
      ...
      const d = {key: moment(b2.key_as_string).format('LLL'), count: b2.doc_count};
...
const getFieldedHistogram = async (fieldName, dateRangeStr, interval = '1h') => {
  const searchBody = {
    index: "partnerdata",
    type: "partnerarticle",
    body: {
