...
const getDetailedFieldedHistogram = async (
  ...
  // console.log(fieldValue + JSON.stringify(histogram, null, 2))
...
const getHistogram = async (dateRangeStr, duration) => {
  ...
  for (let i = 0; i < buckets.length; i++) {
    ...
    const entry = {
      count: b.doc_count,
      label: moment(b.key_as_string).format('LLLL')
    };
...
const getCheckedHistogram = async (dateRangeStr, duration) => {
  const histogram = await client.search({
    index: "checked",
    type: "checkedarticle",
    body: {
...
exports.getCheckedHistogram = getCheckedHistogram;
