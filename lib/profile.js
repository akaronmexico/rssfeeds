	...
	const summaryPercolate = await elastic.percolate(item.summary);
	 ...
	 if (summaryPercolate.hits.total > 0) {
		...
		// console.log("*************************************");
		 if(summaryPercolate.hits.hits) {
			summaryPercolate.hits.hits.forEach( hit => {
					summaryHits++;
				...
				//	console.log("score: " + JSON.stringify(hit,null,2));
				matches.push({item: item, partner: partner, target: target, keyword: keyword});
	 ...
	 if (titlePercolate.hits.total > 0) {
		 ...
		 if(titlePercolate.hits.hits) {
			titlePercolate.hits.hits.forEach( hit => {
	 ...
	 console.log("# of matches:\t" + matches.length + "\ttitle hits:\t" + titleHits + "\t" + item.title);
	 // need async way to insert all matches into partnerTables...
