...
const profileItem = async (item, io) => {
  ...
  //  console.log("summary: " + JSON.stringify(summaryPercolate, null, 2))
  if (summaryPercolate.hits && summaryPercolate.hits.total && summaryPercolate.hits.total.value > 0) {
    // uncomment to see what percolate hits from elastic look like...
    console.log("*******\t" + item.title + "   matched " + " max score: " + summaryPercolate.hits.max_score);
  ...
  if (titlePercolate.hits && titlePercolate.hits.hits && titlePercolate.hits.total > 0) {
