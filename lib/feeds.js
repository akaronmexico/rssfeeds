...
const setupFeeder = async () => {
  ...
  feeder.on("new-item", async item => {
    ...
    // console.log(moment().format('LLLL') + " new article: " + item.title);
    ...
    // console.log("media thumbnail: " + JSON.stringify(item, null, 2));
...
const addFeeds = async () => {
  try {
    let rows = await db.all("SELECT * FROM sources order by id desc");
    for (let row of rows) {
      feeder.add({
        url: row.url
      });
