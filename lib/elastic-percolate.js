...
const rebuildMetrics = async () => {
  ...
  const sql =
    "SELECT partnerdata.*, partners.region, partners.subregion, bins.bin FROM partnerdata JOIN partners ON partners.partner = partnerData.partner JOIN bins ON bins.id = partnerdata.binId order by score desc LIMIT 5 ";
    ...
    for (let article of rows) {
      let doc = {
        ...
        bulk.push({index: {_index: "partnerdata", _type: "partnerarticle", _id: article.uuid}});
        bulk.push(doc);            
    }
    console.log("indexing " + rows.length + " articles");
    const bulkRet = await client.bulk({body: bulk});
    let errorCount = 0;
    console.log(JSON.stringify(bulkRet,null,2));
    bulkRet.items.forEach(item => {
