...
exports.getPartnerBoards = async () => {
  ...
  try {
    ...
    for (let partner of rows) {
      boards.push({ name: partner.partner, id: uuidv1() });
    }
    boards.push({ name: "Aggregated Partner View", id: uuidv1() });
  } catch (err) {
    console.log(err);
  }
...
exports.getPartnerData = async partner => {
  let board = {};
  board.name = "Aggregated Partner View";
  board.id = uuidv1();
  board.cards = [];
  let lists = {};
  let rows = null;
  let sql = "";
  if (partner && partner === "Aggregated Partner View") {
    partner = null;
  }
  if (partner && partner !== "") {
