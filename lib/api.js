...
exports.createPartnerConfig = async (body) => {
 ...
 let sql = "insert into partners (partner, nativeName, capital, region, subregion, avatar, timestamp) values (?,?,?,?,?,?,?)";
    let results = await db.run(sql,[body.name, body.nativeName, body.capital, body.region, body.subregion, body.avatar, new Date().toISOString()]);
	...
	if(bins && targetResults && targetResults.lastID) {
	  ...
	  for(let j = 0; j < bins.length; j++) {
	    ...
	    console.log("binId: " + binId + " targetId:  "+ targetId);
	    const keywords = bin.keywords;
	    if(keywords) {
	      for(let k = 0; k < keywords.length; k++) {
		const keyword = keywords[k];
		if(keyword) {
		  sql = "insert into keywords (binId, targetId, keyword) values (?,?,?)";
