...
exports.getDetailedSourceHistogram = async rssId => {
  let sql = "select url from sources where id = ?";
  let rows = await db.all(sql, [rssId]);
  if (rows && rows.length > 0) {
    let rssUrl = rows[0].url;
    return await elastic.getDetailedSourceHistogram(rssUrl, "7d/d", "1d");
  } else {
    return [];
  }
};

exports.getDetailedPartnerHistogram = async partnerId => {
  let sql = "select url from partners where uuid = ?";
  let rows = await db.all(sql, [partnerId]);
  if (rows && rows.length > 0) {
    let partnerName = rows[0].url;
    return await elastic.getDetailedPartnerHistogram(partnerName, "7d/d", "1d");
  } else {
    return [];
  }
};
...
exports.getArticleHistogram = async () => {
  return await elastic.getHistogram("7d/d", "1h");
};
exports.getCheckedHistogram = async () => {
  return await elastic.getCheckedHistogram("7d/d", "1h");
};
exports.getPartnerHistogram = async () => {
  return await elastic.getFieldedHistogram("partner", "7d/d", "1h");
};
exports.getSourceHistogram = async () => {
  return await elastic.getFieldedHistogram("rssUrl", "7d/d", "10m");
};
exports.getTargetHistogram = async () => {
  return await elastic.getFieldedHistogram("target", "7d/d", "1h");
};

exports.getTopTerms = async () => {
  return await elastic.getTermsAgg("keywords", null);
};

exports.getPartnerTopTerms = async () => {
  return await elastic.getTermsAgg("partner", null);
};

exports.getSourceTopTerms = async () => {
  return await elastic.getTermsAgg("source", null);
};

exports.getTargetTopTerms = async () => {
  return await elastic.getTermsAgg("target", null);
};
...
exports.getPartnerConfig = async () => {
  ...
  let countSql = "select count(*) from partnerdata where partner=?";
  ...
  if (partners) {
    for (let i = 0; i < partners.length; i++) {
      ...
      let histogram = await elastic.getDetailedFieldedHistogram(
        "partner",
        partner.name,
        "7d/d",
        "1h"
      );
      let countRet = await db.all(countSql, [partner.name]);
      if (!countRet || countRet.length === 0) {
        partner.count = 0;
      } else {
        partner.count = Object.values(countRet[0])[0];
      }

      partner.histogram = histogram;
      partner.targets = [];
...
exports.getFeedCount = async () => {
  let sql = "select count(*) from sources";
  let count = await db.all(sql);
...
exports.getTitleCount = async () => {
  let sql = "select count(*) from titles";
  let count = await db.all(sql);
  if (!count || count.length === 0) {
    return 0;
  }
...
exports.getArticleCount = async () => {
  let sql = "select count(*) from partnerdata";
  let count = await db.all(sql);
  if (!count || count.length === 0) {
    return 0;
  }
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (histogram && histogram.length > 0) {
        feed.count = histogram[0].count;
      } else {
        feed.count = 0;
      }

      feed.histogram = histogram;
      feeds.push(feed);
    }
    return feeds;
  } catch (err) {
