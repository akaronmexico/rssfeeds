...
exports.getPartnerConfig = async () => {
  let sql = "select distinct id, partner, capital, region, subregion, avatar, nativeName from partners";
  ...
  if(partners) {
	...
	for(let j = 0; j < targets.length; j++) {
	  ...
	  target.bins = [];
	  const bins = {};
	  sql = "select k.id as keywordId, k.keyword as keywordName, b.id as binId, b.bin as binName, b.description as binDescription from keywords k, bins b, targets t where k.binId = b.id and k.targetId = t.id and t.id=?";
