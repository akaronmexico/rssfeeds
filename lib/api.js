...
exports.createPartnerConfig = async (body) => {
  ...
  try {
     await db.run(sql[(body.name, body.nativeName, body.capital, body.region, body.subregion, body.latlng[0], body.latlng[1], body.avatar, new Date().toISOString())],
    function(err) {
      if (err) {
        console.log(err.message);
      }
      let partnerId = this.lastID;
      sql = "insert into targets (partnerId, target) values (?,?)";
      for(let i = 0; i < targets.length; i++) {
	const target = targets[i];
	 await db.run(sql[(partnerId, target.name)],
    function(err) {
      if (err) {
        console.log(err.message);
      }
      let targetId = this.lastID;
