...
exports.getArticleHistogram = async () => {
  return await elastic.getHistogram(null, null);
};
exports.getPartnerHistogram = async () => {
  return await elastic.getFieldedHistogram('partner','7d/d','1d');
};
exports.getSourceHistogram = async () => {
  return await elastic.getFieldedHistogram('rssUrl','7d/d','1m');
};
exports.getTargetHistogram = async () => {
  return await elastic.getFieldedHistogram('target','7d/d','1d');
};
...
exports.getPartnerConfig = async () => {
  ...
  if (partners) {
    for (let i = 0; i < partners.length; i++) {
      ...
      let histogram = await elastic.getDetailedFieldedHistogram('partner',partner.name,'7d/d', '1m');
      if(histogram && histogram.length > 0) {
	partner.count = histogram[0].count;
      } else {
	partner.count = 0;
      }
...
exports.getFeeds = async () => {
  try {
    ...
    for(let i = 0; i < rows.length; i++) {
      let feed = rows[i];
      let histogram = await elastic.getDetailedFieldedHistogram('rssUrl',feed.url, '7d/d', '1s');
      if(histogram && histogram.length > 0) {
