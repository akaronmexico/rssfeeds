...
exports.getPartnerBoards = async () => {
  ...
  try {
    ...
    for (let partner of rows) {
      boards.push({ name: partner.partner, id: uuidv1(), uri: partner.partner, settings: {color: 'fuse-dark', subscribed: false, cardCoverImages:  false }});
    }
    boards.push({ name: "Aggregated Partner View", id: uuidv1(), uri: 'all', settings: {color: 'fuse-dark', subscribed: false, cardCoverImages:  false } });
  } catch (err) {
    console.log(err);
  }
...
exports.getPartnerData = async (partner) => {
  ...
  board.settings = {color: 'fuse-dark', subscribed: false, cardCoverImages:  false };
  let lists = {};
  let rows = null;
  let sql = "";
