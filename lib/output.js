...
const outputLoop = async results => {
  ...
  for (let partner of results) {
    ...
    sql = "SELECT * FROM partnerdata where partner = ? and lower(status) = ?";
    try {
      let rows = await db.all(sql, [partner, 'new']);
      let output =
        "Target country | Bin | Keyword(s) | Title | Summary | Link | Published | Compiled | Source | feedname | Tracking Url";
