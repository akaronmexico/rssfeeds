...
const fillTable = async (
  ...
  try {
    let sql =
      "select score,title,summary,link from partnerdata where partner=? and title=? and summary=? and link=?";
    ...
    if (results.length < 1) {
      let uuid = uuidv4();
      let sql =
        "insert into partnerdata (uuid, partner, target, keywords, title, summary, link, published, timestamp, runtime, src, feedname, currentflag, score, status, binId) \
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
      let post = [
        uuid,
        partner,
      ...
      console.log(partner + " article (" + keyword + "):\t" + article.title);
