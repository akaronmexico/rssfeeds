...
const fillTable = async (article, score, partner, target, keyword, now) => {
    ...
    try {
        ...
        if (results.length < 1) {
            let sql = 'insert into partnerdata (partner, target, keywords, title, summary, link, published, timestamp, runtime, src, feedname, currentflag, score, status) \
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
            let post  = [partner,target, keyword, article.title, article.summary, article.link,
                article.published, article.timestamp, now, article.src, article.feedname, 1, score, 'new'];
