...
const statuses = ['New','In Work', 'Amplied','Published', 'New', 'New', 'In Work', 'New', 'Published', 'Amplified', 'In Work', 'New']
...
const fillTable = async (article, score, partner, target, keyword, now, binId) => {
    await s.acquire();
    try {
        let sql = 'select score,title,summary,link from partnerdata where partner=? and title=? and summary=? and link=?';
        let results = await db.all(sql, [partner, article.title, article.summary, article.link]);
        if (results.length < 1) {
            let sql = 'insert into partnerdata (partner, target, keywords, title, summary, link, published, timestamp, runtime, src, feedname, currentflag, score, status, binId) \
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
            let post  = [partner,target, keyword, article.title, article.summary, article.link,
                article.published, article.timestamp, now, article.src, article.feedname, 1, score, statuses[Math.floor(Math.random()*statuses.length)], binId];
